...
export default async function ReferralPage() {
  ...
  return (
    <div className={styles.content}>
      <style>{`
        .statsGrid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 1.25rem; margin-bottom: 2rem; }
        .flexRowMobileCol { display: flex; gap: 0.5rem; }
        .historyRow { display: flex; justify-content: space-between; align-items: center; padding: 1rem 1.25rem; background: var(--bg-primary); border-radius: 10px; border: 1px solid var(--border); }
        .payoutRow { margin-top: 1rem; display: flex; align-items: center; justify-content: space-between; background: rgba(16,185,129,0.05); padding: 1rem; border-radius: 10px; border: 1px solid rgba(16,185,129,0.2); }
        
        @media (max-width: 768px) {
          .statsGrid { grid-template-columns: 1fr 1fr; }
          .flexRowMobileCol { flex-direction: column; }
          .historyRow { flex-direction: column; align-items: flex-start; gap: 0.75rem; }
          .payoutRow { flex-direction: column; align-items: flex-start; gap: 0.75rem; }
        }
      `}</style>
      ...
      <div className={styles.card} style={{ marginBottom: '1.5rem', background: 'linear-gradient(135deg, rgba(99,102,241,0.08), rgba(16,185,129,0.08))', border: '1px solid rgba(99,102,241,0.2)' }}>
        ...
        <div className="flexRowMobileCol">
          <input
            type="text"
            readOnly
      ...
      <div className={styles.card} style={{ marginBottom: '1.5rem' }}>
        ...
        <form action={saveUpiId}>
          <div className="flexRowMobileCol" style={{ maxWidth: '500px' }}>
            <input
              type="text"
              name="upi_id"
        ...
        <div className={styles.card} style={{ marginBottom: '1.5rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {withdrawals.map((wd: any) => (
              <div key={wd.id} className="historyRow">
                <div>
        ...
        {referrals.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            {referrals.map((ref: any) => (
              <div key={ref.id} className="historyRow">
                <div>
