...
import { Zap, Play, Pause, Trash2, AlertTriangle, Pencil } from "lucide-react";
...
export default async function AutomationsList({ searchParams }: PageProps) {
  ...
  if (user?.plan && automations.length > 0 && selectedAccountId) {
    ...
    if (activeToken) {
      try {
        const resp = await fetch(`https://graph.instagram.com/v21.0/me/media?fields=id,media_url,thumbnail_url,caption&limit=100&access_token=${activeToken}`, { next: { revalidate: 60 } });
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        {automations.length === 0 ? (
          ...
          automations.map((auto) => {
            ...
            return (
              <div key={auto.id} className={styles.card} style={{ display: 'flex', alignItems: 'flex-start', gap: '1.25rem', flexWrap: 'wrap' }}>
                <div style={{ width: 48, height: 48, flexShrink: 0, background: 'var(--bg-primary)', borderRadius: '12px', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: auto.is_active ? 'var(--primary)' : 'var(--text-muted)' }}>
                  <Zap size={20} />
                </div>
                <div style={{ flex: '1 1 200px', minWidth: 0, display: 'flex', flexDirection: 'column' }}>
                  <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: '0.75rem', marginBottom: '0.5rem' }}>
                    <h3 style={{ fontSize: '1.1rem', fontWeight: 600, color: 'var(--text-heading)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  ...
                  <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', marginBottom: '0.4rem', display: 'flex', gap: '0.5rem' }}>
                    <span style={{ fontWeight: 600, color: 'var(--text-main)', width: '70px', flexShrink: 0 }}>Keywords:</span> 
                    <span style={{ flex: 1 }}>{keywords}</span>
                  </p>
                  <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', marginBottom: '0.4rem', display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                    <span style={{ fontWeight: 600, color: 'var(--text-main)', width: '70px', flexShrink: 0 }}>Target:</span> 
                    <span style={{ flex: 1, display: 'flex', flexWrap: 'wrap', gap: '0.5rem', alignItems: 'center' }}>
                      {postTargetText}
                    </span>
                  </p>
                  <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', display: 'flex', gap: '0.5rem' }}>
                    <span style={{ fontWeight: 600, color: '#a855f7', width: '70px', flexShrink: 0 }}>Engine:</span> 
                    <span style={{ color: '#a855f7', fontWeight: 600, flex: 1 }}>Active (Spintax Replies)</span>
                  </p>
                ...
                <div style={{ display: 'flex', gap: '0.5rem', width: '100%', justifyContent: 'flex-end', borderTop: '1px solid var(--border)', paddingTop: '1rem', marginTop: '0.5rem' }}>
                  <Link
