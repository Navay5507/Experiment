...
import { RefreshCw, RotateCcw, CheckCircle2, Clock, AlertTriangle, MessageCircle } from 'lucide-react';
...
export default function CommentQueuePanel() {
  ...
  const handleRetry = async (comment: PendingComment) => {
    const commentId = comment.metadata?.comment_id;
    if (!commentId) return;
    ...
    try {
      ...
      if (res.ok) {
        ...
        setPending(prev => prev.map(p =>
          p.metadata?.comment_id === commentId
                ...
                <div key={cid} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.75rem 1rem', background: 'var(--surface)', borderRadius: '10px', border: '1px solid var(--border)', gap: '0.75rem' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', fontFamily: 'monospace' }}>
                      {new Date(comment.created_at).toLocaleString()} · ID: {String(cid).slice(0, 16)}…
                    </div>
                  </div>
                  <button
