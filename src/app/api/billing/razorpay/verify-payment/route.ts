...
export async function POST(req: Request) {
  try {
    ...
    // ── IDEMPOTENCY CHECK ─────────────────────────────────────────────────────
    // Prevent replay: one payment_id can never fulfil more than once.
    ...
    if (alreadyFulfilled) {
      console.warn(`[Razorpay] ⚠️ Replay detected — payment ${razorpay_payment_id} already fulfilled.`);
    ...
    // Fetch the Razorpay order/payment to verify ownership and capture status
    let order: any = null;
    ...
    try {
      ...
      console.error("[Razorpay] ❌ Could not fetch order/payment:", e);
      return new NextResponse("Could not verify payment with Razorpay", { status: 502 });
    }

    // ── OWNERSHIP CHECK ───────────────────────────────────────────────────────
    // The order must have been created for this exact authenticated user.
    if (order?.notes?.clerk_id && order.notes.clerk_id !== clerkId) {
      console.error(`[Razorpay] ❌ Ownership mismatch — order belongs to ${order.notes.clerk_id}, caller is ${clerkId}`);
      return new NextResponse("Forbidden", { status: 403 });
    }

    // ── PAYMENT STATUS CHECK ──────────────────────────────────────────────────
    // Only fulfil if Razorpay confirms money was actually captured.
    if (payment?.status !== "captured") {
      console.error(`[Razorpay] ❌ Payment status is "${payment?.status}", expected "captured"`);
      return new NextResponse("Payment not captured", { status: 402 });
    }

    // ── WRITE IDEMPOTENCY LOG before fulfilling ───────────────────────────────
    ...
    if (logError) {
      console.error("[Razorpay] ❌ Payment log insert failed (concurrent replay?):", logError.message);
      return NextResponse.json({ error: "Payment already being processed." }, { status: 409 });
    }
