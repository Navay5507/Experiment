...
export async function POST(req: Request) {
  try {
    ...
    // Fetch the Razorpay subscription to get notes (billingCycle, promoCode, trial_days)
    let sub: any = null;
    let trialDays = 0;
    try {
      sub = await razorpay.subscriptions.fetch(razorpay_subscription_id);
      if (sub && sub.notes && sub.notes.trial_days) {
        trialDays = parseInt(sub.notes.trial_days, 10);
      }
    } catch (e) {
      console.error("[RAZORPAY_FETCH_SUB_ERROR]:", e);
    }
    ...
    if (sub && sub.current_end) {
      ...
      expiresAt.setDate(expiresAt.getDate() + 30); // Fallback
    }

    // Extract plan name (default to PRO if missing)
    const planName = (sub && sub.notes && sub.notes.plan_name) ? sub.notes.plan_name.toUpperCase() : "PRO";

    // Update the user's plan in Supabase to with expiry date
    const { error: updateError } = await supabase
      .from("users")
      .update({ plan: planName, subscription_expires_at: expiresAt.toISOString() })
      .eq("clerkId", clerkId);
    ...
    try {
      if (sub) {
        const promoCode = sub.notes?.promo_code;
        if (promoCode) {
    ...
    if (purchaser?.referred_by) {
      ...
      if (referral) {
        // Subscriptions do not have amount on root, it's inside plan. Just fallback to 0 or fetch plan if needed.
