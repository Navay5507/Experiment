...
import { getSpacedDelay, markCommentProcessed, getCommenterDelayMs } from '@/lib/queue/dedup';
...
export async function POST(
  ...
  try {
    ...
    for (const mediaId of mediaIds) {
      try {
        ...
        for (const comment of comments) {
          ...
          const claimed = await markCommentProcessed(comment.id);
          if (!claimed) continue;
          ...
          // Use the unified comment worker payload which automatically chains the DM
    ...
    return NextResponse.json({ success: true, queuedCount, queuedImmediate, queuedDelayed });
  } catch (error: any) {
