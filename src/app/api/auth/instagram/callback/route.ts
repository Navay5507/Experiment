...
export async function GET(req: Request) {
  ...
  // URL-decode the state before decrypting — OAuth redirects can encode the + and / chars
  const decodedState = rawState ? decodeURIComponent(rawState) : null;
  const clerkId = decodedState ? safeDecrypt(decodedState) : null;

  console.log('[IG Callback] rawState present:', !!rawState, '| clerkId resolved:', clerkId ? `${clerkId.substring(0, 10)}...` : 'NULL');

  if (!clerkId) {
    console.error('[IG Callback] ❌ CRITICAL: state parameter failed to decrypt or was missing. Blocking to prevent account cross-contamination.');
  ...
  try {
    ...
    console.log('[IG Callback] Token exchange status:', tokenRes.status, tokenData.error || 'OK');
    ...
    const profileRes = await fetch(
      `https://graph.instagram.com/v21.0/me?fields=user_id,username,name&access_token=${finalToken}`
    );
    const profileData = await profileRes.json();
    console.log('[IG Callback] Profile Data from Meta:', profileData);
    const igHandle = profileData.username || profileData.name || `User_${igUserId}`;
