...
export async function POST(req: Request) {
  ...
  try {
    ...
    let resolvedAutomationId = automationId;
    if (!resolvedAutomationId) {
      // Fallback for older events missing automation_id
      ...
      if (autos && autos.length > 0) {
        resolvedAutomationId = autos[0].id;
      } else {
        return NextResponse.json({ error: 'Missing automationId and no active automations found to fallback to' }, { status: 400 });
      }
    }
    ...
    }, { delay: getRandomDelay(3000, 10000) });
