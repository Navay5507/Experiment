...
export default function Header({ activePath, serverUserId }: HeaderProps) {
  const { userId: clientUserId, isLoaded } = useAuth();
  const userId = serverUserId !== undefined ? serverUserId : clientUserId;
      ...
      <nav className={styles.navbar} style={{ maxWidth: "1200px", margin: "0 auto", padding: "0 1.5rem" }}>
          ...
          {isMobileMenuOpen && (
            <div style={{ display: "flex", gap: "1rem", flexDirection: "column", alignItems: "center", marginTop: "1.5rem", borderTop: "1px solid var(--border)", paddingTop: "1.5rem", width: "100%" }}>
              ...
              {!mounted ? (
                ...
                <Link href="/sign-up" className="premium-btn" style={{ fontSize: "1rem", padding: "0.8rem 1.5rem", width: "100%", textAlign: "center" }} onClick={() => setIsMobileMenuOpen(false)}>Start for free</Link>
              )}
            </div>
          )}
        </div>
        ...
        <div className={styles.authCol}>
          ...
          {!mounted ? (
            ...
            <Link href="/sign-up" className="premium-btn" style={{ fontSize: "0.9rem", padding: "0.6rem 1.5rem" }}>Start for free</Link>
          )}
        </div>
      </nav>
    </div>
