...
import { checkHourlyDMLimit, checkHourlyCommentLimit, getRandomDelay, isDMSentToUser, getSpacedDelay } from './dedup';
...
async function upsertConversation(
  ...
  // Robust update/insert without relying on DB unique constraints
...
// =============================================
const backoffStrategy = (attemptsMade: number, type: any, err: any, job: any) => {
  if (type === 'nextHour') {
    const now = new Date();
    const nextHour = new Date();
    nextHour.setHours(now.getHours() + 1, 0, 0, 0);
    
    // Add a base buffer (5s) + small offset based on job timestamp to preserve FIFO
    const offset = job.timestamp ? (job.timestamp % 10000) : 0;
    return Math.max(0, nextHour.getTime() - now.getTime() + 5000 + offset);
  }
  return 5000; // default
};
...
// =============================================
// PERMANENT GUARD: Workers must NEVER boot inside Vercel serverless functions.
// The !process.env.VERCEL check ensures workers only run on the Render deployment.
export const dmWorker = !process.env.VERCEL ? new Worker('autodrop-queue', async (job: Job<AutomationJob>) => {
  ...
  if (!user || !automation) {
    console.error(`[Worker DM] ABORT: Non-retryable — data deleted from DB. user=${!!user}, automation=${!!automation}`);
    return;
  }
  ...
  // ANTI-BAN: 24-hour deduplication for ALL initial triggers
  // Only allow a person to get the initial DM (or Private Reply) for an automation once per 24 hours.
  if (job.name === 'send' && job.attemptsMade === 0) {
    const isDuplicate = await isDMSentToUser(userId, recipientId, automationId, job.data.mediaId);
    if (isDuplicate) {
      console.log(`[Worker DM] ⚠️ Skipping initial SEND for ${recipientId} on automation ${automationId} (Already sent within 24h)`);
      return; // Skip silently to respect 24-hour limit
    }
  }
  ...
  if (!rateCheck.allowed) {
    ...
    // Throw so BullMQ retries this job — never silently drop a DM
  ...
  if (!hourlyCheck.allowed) {
    ...
    // Throw so BullMQ retries this job after the hour resets — never silently drop a DM
    throw new Error(`Hourly DM cap reached (${hourlyCheck.count}/${hourlyCheck.limit}). Will retry.`);
  }

  // Check moved to the top of the function to avoid consuming rate limits on duplicates.
  ...
  if (job.name === 'follow-verify') {
    // Default to false — only deliver if we can positively confirm they are following
    let isFollowing = false;
    try {
      const isUserFollows = await InstagramAPI.isUserFollowingBusiness(recipientId, token);
      if (typeof isUserFollows === 'boolean') {
        isFollowing = isUserFollows;
        console.log(`[Worker DM] Follow Verify for ${recipientId}: following = ${isFollowing}`);
      } else {
        console.warn(`[Worker DM] Follow Verify returned unexpected value. Defaulting to not following (safe).`);
      }
    } catch (err) {
      // If Instagram API fails, we do NOT deliver the link — safer to miss one than to cheat the follow-gate
      console.error(`[Worker DM] Follow Verify API error — defaulting to not following:`, err);
    }
...
// =============================================
// PERMANENT GUARD: Comment workers must NEVER boot inside Vercel serverless functions.
export const commentWorker = !process.env.VERCEL ? new Worker('comment-reply', async (job: Job<AutomationJob>) => {
  ...
  // ANTI-BAN: Synchronize jobs by checking the DM limit FIRST before sending the comment.
  // We peek the DM limit so we don't falsely increment the counter here.
  if (recipientId) {
    const hourlyDMCheck = await checkHourlyDMLimit(userId, true); // PEEK!
    if (!hourlyDMCheck.allowed) {
      console.warn(`[Worker Comment] ⚠️ Hourly DM cap reached for ${userId}. Delaying comment reply.`);
      // We throw an error so BullMQ will retry in 5 minutes (based on route.ts backoff)
      // This synchronizes the comment reply with the actual DM capacity!
      throw new Error(`Hourly DM cap reached. Delaying comment reply to sync with DM capacity.`);
    }
  }
  ...
  if (!hourlyCommentCheck.allowed) {
    ...
    // Throw so BullMQ retries — never silently drop a comment reply
    throw new Error(`Hourly comment cap reached (${hourlyCommentCheck.count}/${hourlyCommentCheck.limit}). Will retry.`);
  }
  ...
  // ANTI-BAN: We rely on markCommentProcessed in route.ts to prevent duplicate jobs for the same comment.
  // The user can trigger the automation multiple times if they leave multiple comments.
  ...
  // DM is dispatched with a random delay (2-5s) for natural timing
  if (recipientId) {
    try {
      await dmQueue.add('send', {
        ...
        delay: await getSpacedDelay(userId, 'dm', 6000),
        attempts: 12,
        backoff: {
          type: 'nextHour',
        }
  ...
  connection: createRedisConnection(),
  blockingConnection: false,
  stalledInterval: 3600000,         // Check for stalled jobs every 1 hour (saves Upstash commands)
  drainDelay: 60000,                // Poll every 60 seconds when idle (saves Upstash commands)
  concurrency: 5,
  metrics: undefined,
  settings: { backoffStrategy }
}) : null as any;
