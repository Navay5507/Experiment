...
export function safeDecrypt(token: string | null | undefined): string | null {
  ...
  if (token.includes(':') && token.split(':').length === 3) {
    try {
      ...
      // If decryption fails, it might be a plain-text token that coincidentally has colons
      // (extremely unlikely for Instagram tokens but handle gracefully)
      console.warn('[Crypto] Decryption failed, treating token as plain text:', (e as Error).message);
      return token;
    }
  }
